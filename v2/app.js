...
(() => {
  ...
  const icon={heart:'<svg class="heart" viewBox="0 0 24 24"><path d="M6 4h12v17l-6-4-6 4V4Z"/></svg>',compare:'<svg viewBox="0 0 24 24"><path d="M5 5v14M12 3v18M19 5v14M3 8h4M10 16h4M17 10h4"/></svg>'};
  const defaults=()=>({q:'',scoreFrom:'',scoreTo:'',batch:'本科批B段',province:'',region:'',category:'',major:'',own:'',level:'',type:'',tag:'',fee:'',tier:'',line:false,subjectOnly:false,sort:'close'});
  const state={route:'explore',view:'groups',layout:'list',pageSize:24,limit:24,profile:{track:0,mode:'score',value:550,subjects:[],demo:true},filters:defaults(),saved:[],compare:[],stage:false};
  let D, pos={}, matched=[], allMatched=[], distributionRows=[], viewRows=[], detailPromise=null, detailData=null, detailToken=0, openedDetail=null, toastTimer, refreshTimer, searchTimer;
...
function copyAboutQQ(btn) {
  ...
  function groupCard(i,n=0){const g=D.groups[i],s=D.schools[g[0]];return `<article class="result-card" style="--i:${Math.min(n,5)}"><div class="card-top"><div class="card-title"><button class="school-name" data-school="${g[0]}">${esc(s.n)}</button><div class="card-location">${esc(s.prov)} · ${esc(s.city)}<span> / ${esc(s.own)}</span></div></div>${badge(i)}</div><div class="card-group"><strong>${esc(groupLabel(g))}</strong><span>·</span><span>${trackName(g[1])} · ${esc(g[G.req]||'选科未提供')}</span></div><p class="card-majors">${esc(majorNames(i))}${D.links[i].length>4?' 等':''}</p><div class="card-metrics"><div><span>2025 组线</span><strong>${valid(g[G.s25])}<small>${g[G.s25]>0?'分':''}</small></strong></div><div><span>2025 最低位次</span><strong class="rank-num">${valid(g[G.r25])}</strong></div><div><span>2026 计划</span><strong>${valid(g[G.plan])}<small>${g[G.plan]>0?'人':''}</small></strong></div></div><div class="card-meta"><span>${esc(fee(g))}</span>${subjectText(g)}</div><div class="card-actions"><button class="detail-link" data-group="${i}">查看详情 <span aria-hidden="true">↗</span></button>${actions(i)}</div></article>`;}
  function groupRow(i,n=0){
    const g=D.groups[i],s=D.schools[g[0]],eq=pos.equivalent;
    const gap=eq&&!eq.edge&&g[G.s25]>0?eq.score-g[G.s25]:null;
    const tags=(s.tag||[]).filter(t=>['985','211','双一流','双高计划'].includes(t)).slice(0,2);
    return `<tr class="group-row" style="--i:${Math.min(n,5)}"><td class="col-school"><div class="row-title"><button class="school-name" data-school="${g[0]}">${esc(s.n)}</button>${badge(i)}</div><div class="row-meta">${esc(s.prov)} · ${esc(s.own)}${tags.length?' · '+tags.map(esc).join(' / '):''}</div><button class="row-group-link" data-group="${i}">${esc(groupLabel(g))} <span>· ${esc(g[G.req]||'选科待补充')}</span> ↗</button></td><td class="col-major" data-label="组内专业"><p>${esc(majorNames(i,3))}${D.links[i].length>3?' 等':''}</p><span>${valid(g[G.plan])} 人计划 · ${D.links[i].length} 个专业名称组合</span></td><td class="col-score" data-label="2025组线"><strong>${valid(g[G.s25])}</strong><small class="score-gap ${gap!==null&&gap<0?'negative':''}">${gap===null?'分差待核对':gap===0?'同位次等效分持平':(gap>0?'等效分高出 ':'等效分低于 ')+Math.abs(gap)+' 分'}</small></td><td class="col-rank" data-label="2025位次">${valid(g[G.r25])}</td><td class="col-fee" data-label="年学费"><span>${esc(fee(g).replace(' 元/年',''))}</span><small>${g[G.feeMin]>0?'元 / 年':'待核对'}</small></td><td class="col-actions">${actions(i)}<button class="row-open" data-group="${i}" aria-label="查看${esc(s.n)}${esc(groupLabel(g))}详情">↗</button></td></tr>`;
  }
  function schoolCard(row,n){const s=D.schools[row.school],lines=row.groups.map(i=>D.groups[i][G.s25]).filter(v=>v>0),lo=lines.length?Math.min(...lines):null,hi=lines.length?Math.max(...lines):null,majors=new Set(row.groups.flatMap(i=>D.links[i].map(l=>l[0])));return `<article class="result-card school-card" style="--i:${Math.min(n,5)}"><div class="card-top"><div class="card-title"><button class="school-name" data-school="${row.school}">${esc(s.n)}</button><div class="card-location">${esc(s.prov)} · ${esc(s.city)} / ${esc(s.own)}</div></div></div><div class="tag-row">${(s.tag||[]).filter(t=>['985','211','双一流','保研资格','双高计划','省重点'].includes(t)).slice(0,4).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}<span class="tag">${esc(s.typ)}</span></div><p class="card-school-summary">当前条件下有 <b>${row.groups.length}</b> 个专业组，<b>${majors.size}</b> 个专业名称。<br>${esc([...majors].slice(0,3).map(m=>D.dicts.major[m]).join(' · '))}</p><div class="card-metrics"><div><span>筛选范围内 · 2025 组线区间</span><strong>${lo===null?'—':nf(lo)+(lo!==hi?'–'+nf(hi):'')}<small>${lo?'分':''}</small></strong></div><div><span>符合条件的组</span><strong>${row.groups.length}<small>个</small></strong></div></div><div class="card-actions"><button class="detail-link" data-school="${row.school}">查看院校 <span>↗</span></button><span class="tag">${esc(s.lvl)}</span></div></article>`;}
  function majorCard(row,n){const names=[...row.schools].slice(0,3).map(si=>D.schools[si].n);return `<article class="result-card major-card" style="--i:${Math.min(n,5)}"><div class="card-top"><div class="card-title"><button class="school-name" data-major="${row.major}">${esc(D.dicts.major[row.major])}</button><div class="card-location">${esc([...row.categories].map(c=>D.dicts.cat[c]).join(' / '))}</div></div></div><p class="card-school-summary">${esc(names.join('、'))}${row.schools.size>3?' 等院校在当前范围内招生。':'。'}</p><div class="card-metrics"><div><span>符合条件的院校记录</span><strong>${row.schools.size}<small>所</small></strong></div><div><span>包含该专业的组</span><strong>${row.groups.size}<small>个</small></strong></div></div><div class="card-actions"><button class="detail-link" data-major="${row.major}">查看招生院校 <span>↗</span></button></div></article>`;}
  ...
  function setupControls(){
    ...
    $('#data-count').textContent=`${nf(D.groups.length)} 个专业组 · 数据口径 ↗`;
    $('#quick-own').innerHTML=options([...new Set(D.schools.map(s=>s.own).filter(Boolean))],state.filters.own,'全部性质');
    $('#quick-tag').innerHTML=options(Object.keys(D.tags),state.filters.tag,'全部标签');
  ...
  function syncProfile(){
    ...
    $('#demo-label').textContent=state.profile.demo?'示例分数 · 可修改':'已设置定位';
  ...
  function renderFilterChips(){
    ...
    $('#quick-province').value=f.province;$('#quick-batch').value=f.batch;$('#quick-category').value=f.category;$('#quick-own').value=f.own;$('#quick-tag').value=f.tag;$('#quick-fee').value=f.fee;
  ...
  function renderDistribution(){
    ...
    $('#distribution-description').textContent=`${trackName(state.profile.track)} · 当前范围 ${nf(scores.length)} 个有线专业组`;
  ...
  function renderResults(append=false){
    ...
    $('#results-count').innerHTML=`<strong>${nf(viewRows.length)}</strong> ${unit}<span> · ${trackName(state.profile.track)}${state.filters.batch?' / '+esc(state.filters.batch):' / 全部批次'}</span>`;
    const table=state.view==='groups'&&state.layout==='list';
    $('#results').className=table?'results-table':'results-grid';
    $('.layout-toggle').hidden=state.view!=='groups';
    ...
    if(!viewRows.length){$('#results').innerHTML=`<div class="empty-state"><h3>暂未找到符合条件的记录</h3><p>试试移除一个筛选条件，或更换专业关键词。当前分档与选科条件也会影响查询范围。</p><button class="button primary" data-action="empty-reset">重置筛选条件 ↗</button></div>`;}
    else if(table){
      if(append&&$('#group-table-body'))$('#group-table-body').insertAdjacentHTML('beforeend',shown.slice(state.limit-state.pageSize).map((row,n)=>groupRow(row,n)).join(''));
      else $('#results').innerHTML=`<div class="table-scroll"><table class="group-table"><thead><tr><th>院校 / 专业组</th><th>组内专业</th><th>2025 组线</th><th>2025 位次</th><th>招生考试报学费</th><th><span class="sr-only">收藏、对比与详情</span></th></tr></thead><tbody id="group-table-body">${shown.map((row,n)=>groupRow(row,n)).join('')}</tbody></table></div>`;
    ...
    $('#shown-count').textContent=viewRows.length?`已显示 ${nf(shown.length)} / ${nf(viewRows.length)} ${unit}`:'';
  }
  function refresh({reset=true,land=true}={}){
    ...
    distributionRows=C.query(D,{...state.filters,tier:'',scoreFrom:'',scoreTo:''},p);
    const classified=['chong','wen','bao'].reduce((sum,t)=>sum+allMatched.filter(i=>C.tier(pos.rank,D.groups[i][G.r25])===t).length,0);
    $('#other-count').textContent=`另有 ${nf(allMatched.length-classified)} 组属于其他位置或暂无往年线`;
  ...
  function saveToggle(i){const key=D.keys[i],index=state.saved.indexOf(key);if(index>=0){state.saved.splice(index,1);toast('已从备选清单移除');}else{state.saved.push(key);toast('已加入备选。');}storage.write('luodian.v2.saved',state.saved);syncSavedButtons();if(state.route==='saved')renderSaved();}
  function syncSavedButtons(){$('#saved-count').textContent=state.saved.length;$$('[data-save]').forEach(b=>{const i=Number(b.dataset.save),on=state.saved.includes(D.keys[i]);b.setAttribute('aria-pressed',String(on));if(b.classList.contains('button'))b.innerHTML=icon.heart+(on?'已加入备选':'加入备选');else b.setAttribute('aria-label',(on?'取消收藏':'收藏')+D.schools[D.groups[i][0]].n+groupLabel(D.groups[i]));});}
  ...
  function renderSaved(){
    ...
    if(!state.saved.length){$('#saved-results').innerHTML='<div class="empty-state"><div class="empty-mark">♡</div><h3>先保留选项，再慢慢比较。</h3><p>点击专业组旁的收藏标记，把关注的选项保存在这里。</p><button class="button primary" data-route="explore">返回院校与专业 ↗</button></div>';return;}
  ...
  function positionComparison(i){
    ...
    const rankMin=Math.max(1,Math.floor(Math.min(pos.rank,g[G.r25])*.9)),rankMax=Math.ceil(Math.max(pos.rank,g[G.r25])*1.1),myPin=(pos.rank-rankMin)/(rankMax-rankMin)*100,theirPin=(g[G.r25]-rankMin)/(rankMax-rankMin)*100;
    return `<div class="position-compare"><div class="comparison-numbers"><div><small>你的 2025 同位次等效分</small><strong>${eq?(eq.edge?'≥':'')+eq.score:'—'}</strong><small>当前位次 ${nf(pos.rank)}</small></div><div class="gap"><small>同口径分差</small><strong>${gap===null?'—':(gap>0?'+':'')+gap}</strong><span class="tier-badge ${b.color}">${b.name}</span></div><div><small>该组 2025 调档线</small><strong>${valid(g[G.s25])}</strong><small>最低位次 ${valid(g[G.r25])}</small></div></div><div class="comparison-scale" aria-hidden="true"><i class="comparison-pin" style="left:${myPin}%"></i><i class="comparison-pin theirs" style="left:${theirPin}%"></i><span>${nf(rankMin)} 名</span><span>${nf(rankMax)} 名</span></div><p>${ahead?'你的当前位次靠前':'你的当前位次靠后'} ${nf(Math.abs(pos.rank-g[G.r25]))} 名。${b.description}，是位置对照，不是录取概率。刻度为当前位次窗口。${eq?.edge?'等效分在表格边界外，不给出精确分差。':''}</p></div>`;
  }
  ...
  function bind(){
    document.addEventListener('click',e=>{
      ...
      if(b.dataset.layout){state.layout=b.dataset.layout;storage.write('luodian.v2.layout',state.layout);renderResults();updateURL();return;}
      if(b.dataset.scoreFrom!==undefined){const same=String(state.filters.scoreFrom)===b.dataset.scoreFrom;state.filters.scoreFrom=same?'':b.dataset.scoreFrom;state.filters.scoreTo=same?'':b.dataset.scoreTo;refresh({land:false});return;}
      ...
      if(b.dataset.clearFilter){const k=b.dataset.clearFilter;if(k==='scoreBand'){state.filters.scoreFrom='';state.filters.scoreTo='';}else state.filters[k]=defaults()[k];refresh();return;}
    ...
    for(const [id,key] of [['quick-province','province'],['quick-batch','batch'],['quick-category','category'],['quick-own','own'],['quick-tag','tag'],['quick-fee','fee'],['sort','sort']])$('#'+id).addEventListener('change',e=>{state.filters[key]=e.target.value;refresh();});
    ...
    $('#distribution-chart').addEventListener('pointerover',e=>{const b=e.target.closest('[data-score-from]');if(b)$('#distribution-readout').textContent=`2025 年 ${b.dataset.scoreFrom}–${Number(b.dataset.scoreTo)-1} 分：${nf(Number(b.dataset.binCount))} 个专业组。点选可筛选，再次点选取消。`;});
  ...
  function restore(){
    ...
    for(const k of ['q','province','region','category','major','own','level','type','tag','fee','batch','scoreFrom','scoreTo'])if(p.has(k))state.filters[k]=p.get(k);
    ...
    state.layout=(p.get('layout')||storage.read('luodian.v2.layout','list'))==='cards'?'cards':'list';
    if(state.filters.scoreFrom!==''&&(!Number.isFinite(Number(state.filters.scoreFrom))||!Number.isFinite(Number(state.filters.scoreTo))||Number(state.filters.scoreFrom)>=Number(state.filters.scoreTo))){state.filters.scoreFrom='';state.filters.scoreTo='';}
  ...
  async function init(){
    ...
    }catch(e){console.error(e);$('#results').setAttribute('aria-busy','false');$('#results').innerHTML=`<div class="empty-state"><h3>暂时无法读取数据</h3><p>${esc(e.message)}。请检查网络后刷新；也可以先打开第一代。</p><a class="button primary" href="./">重新加载 ↗</a> <a class="button" href="../">打开第一代</a></div>`;$('#results-count').textContent='暂时无法加载';$('#position-note').textContent='数据尚未就绪';$('#distribution-chart').innerHTML='<div class="chart-loading">暂时无法显示数据分布。</div>';$('#data-count').textContent='数据加载未完成';}
  }
  init();
})();
